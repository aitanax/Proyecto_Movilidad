...
s.t. tiempoEmergencia{i in PARKING, j in DISTRITO}: tiempo_llegada[i, j] * decision[i, j] <= 35 * decision[i, j];
...
s.t. LxmnrLx{i in PARKING, k in PARKING: k <> i}: sum{j in DISTRITO} decision[i,j] <= 1.5 * sum{j in DISTRITO} decision[k,j];
